...
if (phantom.args.length !== 3) {
    console.error("Usage: converter.js source dest options");
    phantom.exit();
} else {
...
function convert(source, dest, options) {
    ...
    if (source.indexOf('<svg') === 0) {
        ...
        page.content = source;
        renderToFile(page, dest, options);
...
function renderToFile(page, dest, options) {
    try {
        var dimensions = getSvgDimensions(page);
        page.viewportSize = {
            width: Math.round(dimensions.width * options.scale),
            height: Math.round(dimensions.height * options.scale)
        };
        if (dimensions.shouldScale) {
            page.zoomFactor = options.scale;
        }
    } catch (e) {
